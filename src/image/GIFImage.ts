import { Base64 } from '../io/Base64.js'
...
export class GIFImage {
    ...
    public setPixel(x: number, y: number, pixel: number): void {
        if (x < 0 || this.width <= x) throw new Error('!' + x)
        if (y < 0 || this.height <= y) throw new Error('!' + y)
    ...
    public toDataURL(): string {
        ...
        const s = bytes.reduce((t, v) => t + String.fromCharCode(v), '')
