...
const TypeNumber = Array(41)
    .fill(0)
    .map((_, i) => i) as unknown as { [P in TypeNumber]: P; }
...
export class QRCodeMinimal {
  ...
  private getData(index: number): QRData {
    ...
    public make(): void {
        if (this.typeNumber < 1) {
            ...
            for (; typeNumber < 40; typeNumber++) {
                ...
                this.qrDataList.forEach(data => {
                    buffer.put(data.getMode(), 4)
                    buffer.put(data.getLength(), data.getLengthInBits(typeNumber))
                    data.write(buffer)
                })

                const totalDataCount = rsBlocks.reduce((t, v) => t + v.getDataCount(), 0)
    ...
    private static createBytes(buffer: BitBuffer, rsBlocks: RSBlock[]): number[] {
        ...
        const dcdata: number[][] = new Array(rsBlocks.length).map(_ => [])
        const ecdata: number[][] = new Array(rsBlocks.length).map(_ => [])
        ...
        const totalCodeCount = rsBlocks.reduce((t, v) => t + v.getTotalCount(), 0)
